...
Country* createCountry(char *name,char *continent,int population){
    Country *country=(Country*)malloc(sizeof(Country)); //we allocate the memory for the country
    country->name = (char *)malloc(sizeof(char)*strlen(name)+1); //we allocate the memory for the name of the country
    strcpy(country->name,name); //we copy the name of the country into the country
    country->continent = (char *)malloc(sizeof(char)*strlen(continent)+1);//we allocate the memory for the name of the continent
    strcpy(country->continent,continent);//we copy the continent of the country into the country
    country->population=population; //we copy the population of the country into the country
