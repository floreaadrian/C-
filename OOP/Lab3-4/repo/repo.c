...
Repo *createRepo() {
  Repo *v =
      (Repo *)malloc(sizeof(Repo)); // allocating the space for the repository
  v->countries =
      createArrays(2); // creating the dynamic array for the repository
  return v;
}
...
void destroyRepo(Repo *v) {
  if (v == NULL)
    return;
  destroy(v->countries); // we first need to destroy all the countries from our
                         // dynamic array
...
int addCountry(Repo *v, Country *co) {
  ...
  // first search for a Country with the same name as the one that is to be
  // added and do not add it if it already exists
...
int checkContinent(char *conti) {
  ...
  if (!strcmp(conti, "Europa") || !strcmp(conti, "Africa") ||
      !strcmp(conti, "America") || !strcmp(conti, "Australia") ||
      !strcmp(conti, "Asia"))
    return 1;
  return -1;
}
...
int updateCountry(Repo *v, char *name, char *newName, char *newContinent,
  ...
  // we add the country and get ok which is 1 adding was succsesfull and 0
  // otherwise
  int ok = addCountry(v, co);
  // we destroy the country because we already have a copy of it in the
  // repository
  destroyCountry(co);
  return ok;
}
...
Repo *sortRepo(Repo *res, int option) {
  // Arrays *aux=createArrays(1);
  int i, j;
  for (i = 0; i < getRepoLength(res) - 1; i++) {
    for (j = i + 1; j < getRepoLength(res); j++) {
      // we sort increasing by name
      if (option == 1) {
        if (strcmp(getName(getCountry(res->countries, i)),
                   getName(getCountry(res->countries, j))) > 0) {
          exchange(res->countries, i, j);
        }
      }
      // we sort increasing by continent
      if (option == 2) {
        if (strcmp(getContinent(getCountry(res->countries, i)),
                   getContinent(getCountry(res->countries, j))) > 0) {
          exchange(res->countries, i, j);
        }
      }
      // we sort increasing by population
      else if (option == 3) {
        if (getPopulation(getCountry(res->countries, i)) <
            getPopulation(getCountry(res->countries, j))) {
          exchange(res->countries, i, j);
        }
      }
    }
